...
class OpticalPropagation(OpticalSystem):
    def __init__(
        ...
        # NOW, SAS DOSEN'T WORK WELL.
        # elif propagation == 'sas': 
        #     # self.sas_rev_calculate_grids(zs=[prop_distance]) 
    ...
    def forward(self):
        ...
        field = self.prop(src_field)
        if isinstance(field, list) or isinstance(field, tuple): # SASPropagation.
            field, pixel_size = field
            # self.pixel_size = pixel_size
            # self.Lx, self.Ly = pixel_size[0] * H, pixel_size[1] * W
            # self.grid, self.f_grid = self.set_grid(), self.set_fgrid()
            print(field)
        print(f"Field's shape after propagation: {field.shape}")    
        out = self.sensor(field)
        print(f"Output Field's shape : {out.shape}")
        return unpowered_src_field, src_field, out
...
if __name__ == "__main__":
    ...
    visualize(file_name_format.format('source', torch.round(lamb0[0]/Prop.nanometers)), src_field[0, 0], title=title_format.format("Source", torch.round(lamb0[0]/Prop.nanometers)), mode='abs')
    visualize(file_name_format.format('source', torch.round(lamb0[1]/Prop.nanometers)), src_field[0, 1], title=title_format.format("Source", torch.round(lamb0[1]/Prop.nanometers)), mode='abs')
    visualize(file_name_format.format('source', torch.round(lamb0[2]/Prop.nanometers)), src_field[0, 2], title=title_format.format("Source", torch.round(lamb0[2]/Prop.nanometers)), mode='abs')
